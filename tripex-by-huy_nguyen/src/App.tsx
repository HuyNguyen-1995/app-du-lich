...
import { Trip, Participant, Income, Expense, SubGroup, Hotel, FixedGroup, EstimatedExpense, ScheduleItem } from './types';
...
import { Settings, Wallet, Receipt, Calculator, Map, LogOut, Bed, Lock, Unlock, Users, ClipboardList, Calendar } from 'lucide-react';
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const unsubSchedules = onSnapshot(collection(db, `trips/${tripId}/schedules`), (snap) => {
      const schs: ScheduleItem[] = [];
      snap.forEach(d => schs.push({ id: d.id, ...d.data() } as ScheduleItem));
      setSchedules(schs);
    }, (error) => handleFirestoreError(error, OperationType.GET, `trips/${tripId}/schedules`));
  ...
  const handleSchedulesChange = async (newSchedulesOrUpdater: React.SetStateAction<ScheduleItem[]>) => {
    if (!tripId) return;
    const resolvedSchedules = typeof newSchedulesOrUpdater === 'function' ? newSchedulesOrUpdater(schedules) : newSchedulesOrUpdater;
    setSchedules(resolvedSchedules);

    try {
      const schedulesRef = collection(db, `trips/${tripId}/schedules`);
      const currentSnap = await getDocs(schedulesRef);
      const existingIds = new Set(currentSnap.docs.map(d => d.id));

      const batch = writeBatch(db);
      resolvedSchedules.forEach((sch) => {
        batch.set(doc(db, `trips/${tripId}/schedules`, sch.id), {
        ...
        existingIds.delete(sch.id);
      });

      existingIds.forEach(id => {
        batch.delete(doc(db, `trips/${tripId}/schedules`, id));
      });
      await batch.commit();
    } catch (e) {
      console.error("Error saving schedules:", e);
      handleFirestoreError(e, OperationType.UPDATE, `trips/${tripId}/schedules`);
    }
  };

  const tabs = [
    { id: 'setup', label: 'Cài đặt', icon: <Settings className="w-5 h-5" /> },
    { id: 'plan', label: 'Kế hoạch', icon: <Calendar className="w-5 h-5" /> },
      ...
      <main className="flex-1 overflow-y-auto p-2 sm:p-6 bg-slate-50/50 pb-[calc(1rem+env(safe-area-inset-bottom))]">
           <div className="max-w-5xl mx-auto space-y-4 sm:space-y-6 pb-6">
            ...
            {activeTab === 'plan' && (
              <PlanTab 
                trip={trip}
                schedules={schedules} setSchedules={handleSchedulesChange}
                isLocked={trip.isLocked}
                requestUnlock={requestUnlock}
              />
            )}
